...
| 快速初筛与选岗后深度评估 | 已支持；硬条件、事实证据、未知项分开呈现，不使用无依据的数字分数 |
| 公司事实核验 | 已支持；关键结论保留来源和核验日期 |
| JD 匹配分析与 1–2 个针对性问题 | 已支持 |
| 可编辑 HTML 简历与用户下载 PDF | 已支持 |
| 外部申请表辅助填写 | 已支持辅助流程，效果受网站与浏览器能力影响 |
| 申请结果记录与复盘 | 已支持；面试、拒绝、Offer、撤回和有限次数跟进保存在本地 |
| 最终提交 | 必须由用户本人确认和点击 |
...
岗位分层评估、公司事实核验和申请结果复盘的方法借鉴自开源项目 [MadsLorentzen/ai-job-search](https://github.com/MadsLorentzen/ai-job-search)。本仓库按现有职业档案、隐私授权和人工最终提交边界重新实现；没有打包其丹麦招聘网站爬虫、个人简历模板或 Claude 专用命令。上游项目采用 MIT License，版权与许可文本见 [第三方许可](third_party/ai-job-search/LICENSE)。
