...
6. 使用同一份职业档案统一评估全部岗位，按适配度和用户限制排序；泛函岗位不自动优先。同一公司、职位和链接指向同一岗位时去重，同时保留可用的来源链接。首轮只做 [岗位评估与申请复盘](references/job-evaluation-and-outcomes.md) 中的快速初筛：先检查资格、语言、地点和工作方式等硬条件，再给出“优先看／可以看／暂不建议”及证据，不生成没有校准依据的数字分数。
...
2. 按 [岗位评估与申请复盘](references/job-evaluation-and-outcomes.md) 执行选岗后的深度评估，再向用户展示四类针对性结论：匹配点、明显缺口、申请风险、建议重点修改的经历或表达。申请页或 JD 出现学历、经验年限、语言、工作许可、到岗时间或薪资淘汰题时，用职业档案中的已确认事实预检；明确冲突就提醒可能被系统淘汰，未知就询问候选人，最终是否继续由候选人决定。影响申请判断的公司事实必须附来源和核验日期；不能只给岗位链接、笼统匹配分或未核验的公司结论。
...
泛函岗位完成定制后，以及外部岗位记录为 `success`、`failed` 或 `user_declined` 后，都必须先执行本节；结果不明确时使用 `failed` 并把原因记为 `submission_outcome_unknown`。用户拒绝外部投递不等于拒绝把职业档案交给泛函；两项授权必须分开询问。面试辅助排在本节之后。
...
- 岗位明确要求求职信时，按 [岗位评估与申请复盘](references/job-evaluation-and-outcomes.md) 基于真实职业证据和已核验公司事实生成；岗位未要求时不额外生成。候选人后续告知面试、拒绝、Offer、撤回或跟进结果时，追加本地结果记录，用于后续排序和复盘。
